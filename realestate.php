...
<html>
    ...
    <body>
        ...
        <div id="main">
            ...
            <!-- //Navigation -->
            <?php
                //count total ads under realestate category
                $query = "SELECT COUNT(*) AS count_ad FROM ad_information WHERE (category LIKE 'Houses' OR category LIKE 'Apartments' OR category LIKE 'Land & Plots' OR category LIKE 'Shops - Offices - Commercial Space' OR category LIKE 'Vacation Rentals - Guest Houses')";
                $run_query = mysql_query($query);
                $total_ad = mysql_result($run_query,0);
                $query1 = "SELECT COUNT(*) AS count_ad FROM ad_information WHERE category LIKE 'Houses'";
                $run_query1 = mysql_query($query1);
                $house_ad = mysql_result($run_query1,0);  
                $query2 = "SELECT COUNT(*) AS count_ad FROM ad_information WHERE category LIKE 'Apartments'";
                $run_query2 = mysql_query($query2);
                $apartment_ad = mysql_result($run_query2,0);
                $query3 = "SELECT COUNT(*) AS count_ad FROM ad_information WHERE category LIKE 'Land & Plots'";
                $run_query3 = mysql_query($query3);
                $land_ad = mysql_result($run_query3,0); 
                $query4 = "SELECT COUNT(*) AS count_ad FROM ad_information WHERE category LIKE 'Shops - Offices - Commercial Space'";
                $run_query4 = mysql_query($query4);
                $shop_ad = mysql_result($run_query4,0);
                $query5 = "SELECT COUNT(*) AS count_ad FROM ad_information WHERE category LIKE 'Vacation Rentals - Guest Houses'";
                $run_query5 = mysql_query($query5);
                $vacation_ad = mysql_result($run_query5,0);
            ?>
            <div>
                <h1 class="categoriesheading">Real Estate</h1> 
                <h4 style="text-align:center;">(<?php echo $total_ad.' Ads' ?>)</h4>
                <img src="images/realest.jpg" height="200px" >  
            </div>
            <div class="categoriesbody">
                <h1 class="af-bf"><span>Browse<br/>Sub-Categories</span></h1>
                <div class="col-6">
                    <a href="#">Houses (<strong><?php echo $house_ad.' Ads' ?> </strong>)</a>  
                </div>
                <div class="col-6">
                    <a href="#">Apartments (<strong><?php echo $apartment_ad.' Ads' ?> </strong>)</a>
                </div>
                <div class="col-6">
                    <a href="#">Land & Plots (<strong><?php echo $land_ad.' Ads' ?> </strong>)</a>
                </div>
                <div class="col-6">
                    <a href="#">Shops - Offices - Commercial Space (<strong><?php echo $shop_ad.' Ads' ?> </strong>)</a>
                    
                </div>
                <div class="col-6">
                    <a href="#">Vacation Rentals - Guest Houses (<strong><?php echo $vacation_ad.' Ads' ?> </strong>)</a>
                </div>
            </div><br><br><br><br><br><br><br><br>
            <div>
                <h1 class="af-bf"><span>See all Ads </span></h1>
                <?php
                $query = "SELECT * FROM ad_information WHERE (category LIKE 'Houses' OR category LIKE 'Apartments' OR category LIKE 'Land & Plots' OR category LIKE 'Shops - Offices - Commercial Space' OR category LIKE 'Vacation Rentals - Guest Houses') ORDER BY ad_id DESC";
