...
<html>
    ...
    <body>
        ...
        <div id="main">
            ...
            <!-- //Navigation -->
            <?php
                //count ads under vehicles category
                $query = "SELECT COUNT(*) AS count_ad FROM ad_information WHERE (category LIKE 'Cars' OR category LIKE 'Bikes & Scooters' OR category LIKE 'Bicycles' OR category LIKE 'Other Vehicles' OR category LIKE 'Spare Parts')";
                $run_query = mysql_query($query);
                $total_ad = mysql_result($run_query,0);
                $query1 = "SELECT COUNT(*) AS count_ad FROM ad_information WHERE category LIKE 'Cars'";
                $run_query1 = mysql_query($query1);
                $car_ad = mysql_result($run_query1,0);  
                $query2 = "SELECT COUNT(*) AS count_ad FROM ad_information WHERE category LIKE 'Bikes & Scooters'";
                $run_query2 = mysql_query($query2);
                $bike_ad = mysql_result($run_query2,0);
                $query3 = "SELECT COUNT(*) AS count_ad FROM ad_information WHERE category LIKE 'Bicycles'";
                $run_query3 = mysql_query($query3);
                $bicycle_ad = mysql_result($run_query3,0); 
                $query4 = "SELECT COUNT(*) AS count_ad FROM ad_information WHERE category LIKE 'Other Vehicles'";
                $run_query4 = mysql_query($query4);
                $other_ad = mysql_result($run_query4,0);
                $query5 = "SELECT COUNT(*) AS count_ad FROM ad_information WHERE category LIKE 'Spare Parts'";
                $run_query5 = mysql_query($query5);
                $spare_ad = mysql_result($run_query5,0);
            ?>
            <div>
                <h1 class="categoriesheading">Vehicles</h1> 
                <h4 style="text-align:center;">(<?php echo $total_ad.' Ads' ?>)</h4>
                <img src="images/vehicles.jpg" height="200px" >  
            </div>
            <div class="categoriesbody">
                <h1 class="af-bf"><span>Browse<br/>Sub-Categories</span></h1>      
                <div class="col-6">
                    <a href="#">Cars (<strong><?php echo $car_ad.' Ads' ?> </strong>)</a>  
                </div>
                <div class="col-6">
                    <a href="#">Bikes & Scooters (<strong><?php echo $bike_ad.' Ads' ?> </strong>)</a>
                </div>
                <div class="col-6">
                    <a href="#">Bicycles (<strong><?php echo $bicycle_ad.' Ads' ?> </strong>)</a>
                </div>
                <div class="col-6">
                    <a href="#">Other Vehicles (<strong><?php echo $other_ad.' Ads' ?> </strong>)</a>
                </div>
                <div class="col-6">
                    <a href="#">Spare Parts (<strong><?php echo $spare_ad.' Ads' ?> </strong>)</a>
                </div>
            </div><br><br><br><br><br><br><br><br>
            <div>
                <h1 class="af-bf"><span>See all Ads </span></h1>
                <?php
                $query = "SELECT * FROM ad_information WHERE (category LIKE 'Cars' OR category LIKE 'Bikes & Scooters' OR category LIKE 'Bicycles' OR category LIKE 'Other Vehicles' OR category LIKE 'Spare Parts') ORDER BY ad_id DESC";
